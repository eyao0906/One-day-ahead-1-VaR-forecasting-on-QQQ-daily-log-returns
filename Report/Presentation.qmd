...
GARCHX extends the standard GARCH volatility model by adding VIX as the exogenous market-risk variable to the conditional variance equation. The idea is to test whether implied market volatility helps explain the next-day tail risk of QQQ returns.
...
$$
\sigma_{t+1|t}^2 = \omega + \alpha y_t^2 + \beta \sigma_t^2 + \delta x_t
$$
...
Because returns are scaled by 100 in the estimation step, the final VaR is scaled back before being stored.
...
This matches the paper’s SAV spirit: the tail risk today depends on the previous tail level and on the size of the latest market move. Large absolute returns push the next quantile farther into the left tail.
...
This is exactly why the implementation is now better described as **conceptually aligned with the paper, but computationally simplified**.
...
CAViaR is attractive because it targets the quantity we actually care about for risk management: the conditional lower-tail quantile. It can adapt quickly to changing risk conditions without forcing the full return distribution to be correctly specified. But in this project, the current CAViaR result should still be treated as **exploratory rather than final-best**, because the backtest issue is now mainly **exception clustering and weak optimization**, not a broken recursion formula.
...
This matches Lecture 18’s **Nonparametric Quantile-GARCH SD**: forecast next-step volatility from GARCH, then combine it with the empirical residual tail. That is why FHS gives a more realistic left-tail fit than Gaussian GARCH in this project.
...
| 7 | GARCH(1,1) | 13 | 1.53% | 0.148 | 0.189 | 0.148 | Acceptable baseline |
...
- **Model overlap across runners:** the project now has overlapping model families across the base and extension runners, so the workflow story is less clean
- **Bootstrap implementation:** bootstrap models reset `np.random.seed(42)` inside the forecast function, making rolling bootstrap forecasts deterministic
- **GARCHX benchmark:** the new R `garchx` package version is more defensible than the old custom Python version, but it still uses a **Gaussian VaR overlay**, so it is a benchmark rather than a final top model
- **Legacy/custom GARCHX:** earlier Python GARCHX should only be described as experimental / custom-coded
- **CAViaR-SAV:** still lacks a proper **Dynamic Quantile-style adequacy test** and still uses a relatively weak optimization routine compared with the original paper
- **No RiskMetrics-centered final benchmark story:** RiskMetrics is present, but not competitive enough to anchor the final conclusions

**Bottom line:**

The project is already strong on rolling forecasting, model breadth, and backtesting. The main remaining weaknesses are methodological polish rather than the core empirical conclusion.
