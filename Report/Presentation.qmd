...
Let $r_t$ denote the daily log return at time $t$. For one-step-ahead Value-at-Risk at level $\alpha$, the target object is the left-tail conditional quantile $\operatorname{VaR}_{t+1}(\alpha) = q_{t+1\mid t}(\alpha)$, so that ideally
$$
...
$$
r_{t+1} = \mu_{t+1} + \sigma_{t+1} \omega_{t+1},
$$
...
q_t = -v_t.
$$
$r_t$ is the realized return, while $v_t$ is the positive latent tail-risk level that the CAViaR recursion updates. Then negate it to obtain the return-side VaR quantile $q_t = −v_t$.
...
Lecture 18’s **Nonparametric Quantile-GARCH SD**: forecast next-step volatility from Gaussian/Student-t GARCH, then combine it with the empirical residual tail, but instead of using a Gaussian or Student-\(t\) quantile at the end, it uses the **empirical quantile of standardized residuals**. 
$$
\widehat{\operatorname{VaR}}_{t+1}(\alpha) = \hat{\mu}_{t+1} + \hat{\sigma}_{t+1}\hat{q}_{\alpha,\mathrm{emp}}
$$
where $\hat{q}_{\alpha,\mathrm{emp}} = \hat{F}^{-1}(\alpha)$ is the distribution of GARCH residuals.
...
Bootstrap fits GARCH first, then resamples the fitted standardized residuals, simulates many one-step returns, 
$$
...
- **Kupiec unconditional coverage test**: higher is better. Low means the violation rate is too far from the target.
- **Christoffersen independence test**: higher is better. Low means violations are clustering instead of arriving independently.
- **Conditional coverage test**: higher is better. Joint check, high means the model gets both the overall hit rate and the timing pattern more right.

## Forecast Uncertainty: 95% CI for VaR
The common idea is an asymptotic quantile confidence interval:
$$
...
$$
$\hat q_\alpha$ is the fitted tail quantile, $f(\hat q_\alpha)$ is the density at that quantile, $n$ is the effective sample size.

##
For parametric models,
$$
\mathrm{VaR} = \mu + \sigma \hat q_\alpha
$$
where:$\hat q_\alpha = \Phi^{-1}(\alpha)$ for Gaussian models, $\hat q_\alpha = t_{\nu}^{-1}(\alpha) \sqrt{\frac{\nu - 2}{\nu}}$ is the standardized Student-t quantile for Student-t models.
...
## 95% CI: models with different construction
**FHS:** take the empirical $\alpha$-quantile of the standardized residuals, form its sample-quantile 95% CI

**Bootstrap:** resample standardized residuals, generate a simulated one-step return sample, take the empirical $\alpha$-quantile of those simulated returns as VaR, and then build the 95% CI on that simulated-return quantile.

**Historical Simulation:** confidence interval is the standard sample-quantile CI computed on the same rolling window.

**CAViaR-SAV*:**Take the point forecast from the CAViaR recursion, shift a recent return window so its tail quantile matches that forecast, and then apply a sample-quantile 95% CI to that shifted sample.

So CAViaR uses a centered empirical proxy interval, not a fully model-based inferential CI.
...
- Kupiec p checks whether it was wrong the right number of times.
- Christoffersen p checks whether those mistakes were randomly spread out.
- CC p checks both together.
